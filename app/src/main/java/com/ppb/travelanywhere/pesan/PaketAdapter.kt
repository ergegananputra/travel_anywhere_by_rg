...
import com.ppb.travelanywhere.databinding.CardPaketKeretaBinding
...
class PaketAdapter(
    ...
    inner class PaketViewHolder(
        private val binding : CardPaketKeretaBinding
    ) : RecyclerView.ViewHolder(binding.root) {
        fun bind(paketModel: PaketModel) {
            with(binding) {
                textViewPaket.text = paketModel.name
                textViewStatus.text = if (paketModel.status) "Active" else "Inactive"
                textViewDescription.text = paketModel.description

                setIcon(paketModel.id)

                switchPaket.isChecked = paketModel.status
                switchPaket.setOnClickListener {
                    if (switchPaket.isChecked) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): PaketViewHolder {
        val binding = CardPaketKeretaBinding.inflate(
